...
description: >
  Let an agent session hold a real conversation on a channel — receive messages, answer them, and
  hand a message to ANOTHER running agent session — through one CLI, `convo`, with no polling loop
  anywhere. Delivery into a live agent session goes through Herdr (a hard prerequisite), so a
  message lands in a session that is actually idle, and never in one that is blocked on a prompt.
  Use when the user says "listen on a channel", "answer messages from an agent", "wake an agent on
  a message", "deliver a message into a running agent session", "teams / whatsapp / slack bridge
  for agents", "monitor a channel without a polling loop", "reply to channel messages", "route this
  question to another session", "which agents are addressable", "drain the messages nobody
  answered", "herdr agent delivery", "cross-session messaging", "convo", "convo self", "convo host
  deliver", "convo journal", "convo respond", or wires any long-running run to channel reporting
  and control.
---
...
- Runtimes that read `AGENTS.md` instead of a skill dir: `skill/AGENTS.md` is self-contained.
